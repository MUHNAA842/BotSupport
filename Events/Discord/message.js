...
module.exports = async(bot, message) => {
 ...
 if(!message.guild){
   if(message.author.bot) return;
   var guild = bot.guilds.cache.find(g => g.id === "1042040286148296714")
   ...
   if(yourchannel === undefined) {
    ...
    var categoryID = guild.channels.cache.find(channel => channel.name == "Ticket Bot" && channel.type == "category");
     if (!categoryID) {
         guild.channels.create('Ticket Bot', { type: "category" }).then(category => {
